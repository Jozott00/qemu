...
char *vadl_find_bios(const char *bios_filename)
{
    char *filename = NULL;

    filename = qemu_find_file(QEMU_FILE_TYPE_BIOS, bios_filename);
    if (filename == NULL) {
        error_report("Unable to find the VADL BIOS '%s'", bios_filename);
        exit(1);
    }
...
target_ulong vadl_find_and_load_firmware(MachineState *machine,
                                         ...
                                         symbol_fn_t symbol_fn)
{
    ...
    firmware_filename = vadl_find_bios(machine->firmware);

    if (firmware_filename) {
        firmware_end_addr = vadl_load_firmware(firmware_filename, firmware_load_addr, symbol_fn);
    }
